...
node *build (tipo *a, int tl, int tr) { // modificar para tomar tipo a
	if(tl + 1 == tr) return new node(a[tl]);
	int tm = (tl + tr) >> 1;
	return new node(build(a, tl, tm), build(a, tm, tr));
}
node *upd(int pos, int new_val, node *t, int tl, int tr){
	if(tl + 1 == tr) return new node(new_val);
	int tm = (tl + tr) >> 1;
	if(pos < tm) return new node(upd(pos, new_val, t->l, tl, tm), t->r);
	else return new node(t->l, upd(pos, new_val, t->r, tm, tr));
}
...
int main() {
	...
	while(scanf("%s %d %d", c, &a, &b)>0){
		if(tolower(c[0])=='s') root=upd(a, b, root, 0, n);
		else printf("%d\n", get(a, b, root, 0, n));
	}
	return 0;
}
